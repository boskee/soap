...
namespace Boskee\Soap;

/**
 * Boskee\Soap\Server allows the user to create a SOAP server
...
class Server extends Base
{
    ...
    public function __construct($wsdl = false)
    {
        ...
        if (isset($debug)) {
            $this->debug("In Boskee\Soap\Server, set debug_flag=$debug based on global flag");
            $this->debug_flag = $debug;
        } elseif (isset($_SERVER['QUERY_STRING'])) {
            $qs = explode('&', $_SERVER['QUERY_STRING']);
            foreach ($qs as $v) {
                if (substr($v, 0, 6) == 'debug=') {
                    $this->debug("In Boskee\Soap\Server, set debug_flag=".substr($v, 6).' based on query string #1');
            ...
            foreach ($qs as $v) {
                if (substr($v, 0, 6) == 'debug=') {
                    $this->debug("In Boskee\Soap\Server, set debug_flag=".substr($v, 6).' based on query string #2');
                    $this->debug_flag = substr($v, 6);
                }
            }
        }
        ...
        // wsdl
        if ($wsdl) {
            $this->debug("In Boskee\Soap\Server, WSDL is specified");
            if (is_object($wsdl) && (get_class($wsdl) == 'Boskee\Soap\WSDL')) {
                $this->wsdl = $wsdl;
    ...
    public function serialize_return()
    {
        ...
        // if fault
        if (isset($this->methodreturn) && is_object($this->methodreturn) && ((get_class($this->methodreturn) == 'Boskee\Soap\Fault'))) {
